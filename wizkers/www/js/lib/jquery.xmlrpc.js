...
(function($) {
	...
	var xmlrpc = $.xmlrpc = function(url, settings) {
		...
		settings.converters = {'xml json': xmlrpc.parseResponse};
	...
	*/
	...
	/**
	* Make an XML-RPC method response document from` a set of parameters
	*/
	xmlrpc.response = function(params) {
		var doc = xmlrpc.createXMLDocument();
		...
		var $methodCall = $xml('methodResponse').append($params);
		doc.appendChild($methodCall.get(0));
		return doc;
	};
	...
	*/
	...
	/**
	* Take an XML-RPC method call document and decode it to an equivalent JavaScript
	* representation.
	*/
	xmlrpc.parseCall = function(doc) {
		var $doc = $(doc);
		var $response = $doc.children('methodcall');
	...
	*/
	xmlrpc.parseResponse = function(doc) {
		var $doc = $(doc);
