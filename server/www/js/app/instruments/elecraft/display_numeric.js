    ...
    return Backbone.View.extend({
        ...
        initialize:function () {
            ...
            addPlot: function() {
                ...
                this.tempplot= new simpleplot({model: this.model, settings:this.plotoptions});
                if (this.tempplot != null) {
                    $('.tempchart', this.el).append(this.tempplot.el);
                    this.tempplot.render();
                }
                this.amppowerplot= new simpleplot({model: this.model, settings:this.plotoptions});
                if (this.amppowerplot != null) {
                    $('.amppowerchart', this.el).append(this.amppowerplot.el);
                    this.amppowerplot.render();
                }
                this.voltplot= new simpleplot({model: this.model, settings:this.plotoptions});
            ...
            showInput: function(data) {
                ...
                if (data.charAt(0) == '^') {
                    ...
                    if (cmd == "PI") {
                        $("#kxpa-pi").html(val);
                        this.amppowerplot.appendPoint({'name': "In", 'value': val});
                    } else if (cmd == "PF") {
                        $("#kxpa-pf").html(val);
                        this.amppowerplot.appendPoint({'name': "Fwd", 'value': val});
                    } else if (cmd == "PV") {
                        $("#kxpa-pv").html(val);
                        this.amppowerplot.appendPoint({'name': "R", 'value': val});
                    } else if (cmd == "TM") {
                        $("#kxpa-tm").html(val);
                        this.tempplot.appendPoint({'name': "PA.X", 'value': val});
                    } else if (cmd == "PC") {
                        $("#kxpa-pc").html(val);
                        this.voltplot.appendPoint({'name': "A", 'value': val});
                    } else if (cmd == "SV") {
                        var val = Math.floor(val)/100;
                        $("#kxpa-sv").html(val);
                        this.voltplot.appendPoint({'name': "V", 'value': val});
                    }
                } else {
                    ...
                    if (cmd == "PO") {
                        ...
                        this.amppowerplot.appendPoint({'name': "KX3", 'value': val});
                    } else if (cmd == "DB") {
                        ...
                        switch (cmd2) {
                                case "PA.I":
                                    ...
                                    this.tempplot.appendPoint({'name': "PA.I", 'value': val});
                                    break;
                                case "OSC ":
                                case "OSC*":
                                    val = parseInt(data.substr(6,2));
                                    this.tempplot.appendPoint({'name': "OSC", 'value': val});
                                    break;
                                case "PA.2":
                                    val = parseInt(data.substr(7,2));
                                    this.tempplot.appendPoint({'name': "PA.2", 'value': val});
                                    break;
                        }
                    }
                }
